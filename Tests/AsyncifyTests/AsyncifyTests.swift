...
@Test func multipleCallersOneFails() async throws {
  ...
  for result in results {
    switch result {
    case .success: Issue.record("The operation should not succeed.")
...
@Test func concurrencyWithSimultaneousOperations() async throws {
  ...
  let results = try await withThrowingTaskGroup(of: Int.self) { group in
    for task in tasks { group.addTask { try await task.value } }
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
      ...
      Task {
        do {
          ...
          #expect(
            reentrantResult == "Re-entrant success",
            "The re-entrant result should be successful."
          )
        } catch { Issue.record("Re-entrant operation should not fail.") }
      }
    }
  }
  #expect(result == expectedResult, "The result should be equal to the expected result.")
